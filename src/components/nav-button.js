...
const NavButton = ({ to, href, label, icon }) => {
  const id =
    "NavButton_" +
    Math.random()
      .toString(36)
  ...
  const Contents = () => (
    <svg
      ...
      aria-labelledby={id}
    >
      {to && <title id={id}>{label || sketchTitle(to)}</title>}
      {icons[icon]}
    </svg>
  );
...
const NavButtonBar = ({ children, label }) => (
  <nav className={styles.bar} aria-labelledby={label || null}>
    {children}
  </nav>
);
