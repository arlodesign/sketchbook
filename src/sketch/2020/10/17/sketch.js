...
const sketch = function (p) {
  ...
  class Voxel {
    ...
    get size() {
      const { x, y, z } = this;
      const { theta, progress } = p.animLoop;
  ...
  p.setup = function () {
    ...
    for (let x = 0; x < RESOLUTION; x++) {
      for (let y = 0; y < RESOLUTION; y++) {
        for (let z = 0; z < RESOLUTION; z++) {
          if (
            p.dist(RESOLUTION / 2, RESOLUTION / 2, RESOLUTION / 2, x, y, z) <=
            RESOLUTION / 2
          ) {
            voxels.push(
              new Voxel(
    ...
    urlParams = Object.assign(
      {
        cameraDistance: RESOLUTION * (RENDER ? 4 * 2 : 2),
      },
      p.getURLParams()
    );
    cameraDistance = p.createSlider(
      RESOLUTION * (RENDER ? 4 * 2 : 2),
