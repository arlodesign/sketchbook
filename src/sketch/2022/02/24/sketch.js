...
const sketch = function (p) {
  ...
  p.draw = function () {
    ...
    p.stroke(
      ...
      1,
      easeOutQuart(Math.abs(noise())) / 2 + 0.5,
      0.25
    );
    p.strokeWeight(RENDER ? 4 : 1);
    p.translate(p.width / 2, p.height / 2);
    p.translate(p.width * 0.25 * noise1D(0.8), p.height * 0.25 * noise1D(0.6));
    p.rotate(theta + p.lerp(-p.PI, p.PI, Math.abs(noise2D(0.1, 0.2))));

    const radius = p.width * 0.5 * noise();

    for (let d = 0; d < p.TWO_PI; d += 0.1) {
      const lerpVal = noise1D(d);
      lerpVal > -0.5 &&
        p.random() > 0.3 &&
        p.point(...polarToCartesian(0, 0, d, radius, true));
    }
