...
import { easeInOutQuint } from "util/easing";
...
const sketch = function (p) {
  ...
  p.setup = function () {
    ...
    p.colorMode(p.HSB, 1);
    p.background(hue.value(), 0.1, 0.2);
  };
  ...
  p.draw = function () {
    ...
    p.translate(p.width * progress, p.height / 3);
    // p.rotate(theta);
    // p.rectMode(p.CENTER);
    ...
    const points = [
      polarToCartesian(
        ...
        0,
        theta,
        p.lerp(0, p.width, loop * easeInOutQuint(Math.abs(noise2D(0, 0.1)))),
        true
      ),
      polarToCartesian(
        0,
        0,
        theta + (p.TWO_PI / 5) * 1,
        p.lerp(0, p.width, loop * easeInOutQuint(Math.abs(noise2D(0.2, 0.3)))),
        true
      ),
      polarToCartesian(
        0,
        0,
        theta + (p.TWO_PI / 5) * 2,
        p.lerp(0, p.width, loop * easeInOutQuint(Math.abs(noise2D(0.4, 0.5)))),
        true
      ),
      polarToCartesian(
        0,
        0,
        theta + (p.TWO_PI / 5) * 3,
        p.lerp(0, p.width, loop * easeInOutQuint(Math.abs(noise2D(0.6, 0.7)))),
        true
      ),
      polarToCartesian(
        0,
        0,
        theta + (p.TWO_PI / 5) * 4,
        p.lerp(0, p.width, loop * easeInOutQuint(Math.abs(noise2D(0.8, 0.9)))),
        true
      ),
    ];
